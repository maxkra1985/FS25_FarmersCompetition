...
function CompetitionManager:resolveRoundBale125Capacity()
    if self.roundBale125Capacity ~= nil and self.roundBale125Capacity > 0 then return self.roundBale125Capacity, self.roundBale125CapacitySource end
    if g_baleManager == nil or g_fillTypeManager == nil then return nil, "Manager unavailable" end
    local strawFillType = g_fillTypeManager.getFillTypeIndexByName ~= nil and g_fillTypeManager:getFillTypeIndexByName("STRAW") or FillType.STRAW
    if strawFillType == nil or strawFillType == 0 then return nil, "STRAW missing" end
    ...
    for baleIndex, bale in ipairs(g_baleManager.bales or {}) do
        if bale ~= nil and bale.isAvailable ~= false and bale.isRoundbale == true and bale.diameter ~= nil and math.abs(bale.diameter - self.ROUND_BALE_125_DIAMETER) <= self.ROUND_BALE_125_TOLERANCE then
            local capacity = g_baleManager:getBaleCapacityByBaleIndex(baleIndex, strawFillType)
            if capacity ~= nil and capacity > 0 then
                self.roundBale125Capacity = capacity
                self.roundBale125CapacitySource = "BaleManager"
                return capacity, self.roundBale125CapacitySource
            end
        end
    end
...
function CompetitionManager:calculateExpectedFruitOnFarmland(ctx, config, fruitDesc, forcedGrowthState)
    ...
    if string.upper(result.fruitName) == "WHEAT" and fruitDesc.windrowLiterPerSqm ~= nil then
        ...
        local baleCapacity, baleSource = self:resolveRoundBale125Capacity()
    ...
    if string.upper(result.fruitName) == "GRASS" and fruitDesc.windrowLiterPerSqm ~= nil then
        result.expectedGrassLiters = weightedAreaSqm * fruitDesc.windrowLiterPerSqm * harvestMultiplier
        local baleCapacity, _ = self:resolveRoundBale125Capacity()
        if baleCapacity ~= nil and baleCapacity > 0 then result.expectedGrassBales125 = math.floor(result.expectedGrassLiters / baleCapacity)
        else result.expectedGrassBales125 = 1 end
    end
